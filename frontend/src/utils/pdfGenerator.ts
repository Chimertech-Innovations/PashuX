...
export function generateHealthReportPDF(data: ReportPDFData) {
  try {
    ...
    doc.text(`Report Reference ID: ${data.requestId}`, 18, y + 7);
    doc.text(`Date of Scan: ${data.date}`, 18, y + 14);

    if (data.userEmail) {
      doc.text(`Registered Owner: ${data.userName || 'Cattle Owner'} (${data.userEmail})`, 105, y + 7);
    }
    doc.text(`Diagnostic Engine: PASHUX NEURAL VISION (${data.analysisType.toUpperCase()})`, 105, y + 14);
    ...
    if (data.possibleCondition) {
      ...
      doc.text(`${data.possibleCondition}`, 65, y);

      if (data.severity) {
        doc.setTextColor(100, 116, 139);
        doc.setFont('helvetica', 'normal');
        doc.text(`(Severity: ${data.severity.toUpperCase()})`, 135, y);
      }
      y += 7;
    }
    ...
    doc.text('iHERD MOBILE APP -- OFFICIAL HERD MANAGEMENT', 18, y + 7);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
    doc.setTextColor(241, 245, 249);
    doc.text('Track daily body condition scores, milk yields, vaccination routines & health logs on Android.', 18, y + 13);

    doc.setFont('helvetica', 'bold');
    doc.setTextColor(52, 211, 153);
    doc.textWithLink('GET IT ON GOOGLE PLAY STORE: Click to Download iHerd App', 18, y + 19, { url: IHERD_PLAY_STORE_URL });
  ...
  const html = `
    ...
    <head>
      ...
      <style>
        ...
        .app-banner { background: #0f172a; color: white; padding: 15px; border-radius: 12px; margin-top: 25px; }
        .app-banner a { color: #34d399; font-weight: bold; text-decoration: underline; }
    ...
    <body>
      ...
      <div class="app-banner">
        <h3>📱 Track Herd Health on iHerd Mobile App</h3>
        <p>Record daily body condition scores, milk yields, and vaccination logs on your smartphone.</p>
        <a href="${IHERD_PLAY_STORE_URL}" target="_blank">Download iHerd App on Google Play Store →</a>
      </div>
    </body>
    </html>
  `;
