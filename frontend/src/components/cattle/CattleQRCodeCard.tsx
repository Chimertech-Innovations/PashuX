...
export default function CattleQRCodeCard({
  ...
  const shareWhatsApp = () => {
    const text = `🐄 *Cattle Profile Biometrics*\n\n` +
      ...
      `❇️ *Health Status:* ${healthStatus}\n\n` +
      `🛡️ *Powered by Chimertech Innovations*\n` +
      ...
      <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm flex flex-col items-center">
        ...
        <div className="flex gap-2 w-full">
          <button
            ...
            className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs py-1.5 px-2 rounded-xl flex items-center justify-center gap-1 transition-colors"
          >
            <span>📱</span> WhatsApp
          </button>
          <button
            onClick={downloadQR}
            className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs py-1.5 px-2 rounded-xl transition-colors"
      ...
      <div className="flex flex-col md:flex-row gap-8 items-center">
        ...
        <div className="flex-1 w-full space-y-5">
          ...
          <div className="bg-slate-100 border border-slate-200 rounded-xl px-3.5 py-2.5 flex items-center justify-between">
            ...
            <button
              onClick={copyLink}
              className="text-xs font-bold text-emerald-600 hover:text-emerald-700 px-3 py-1 bg-white rounded-lg border border-slate-200 shadow-sm flex-shrink-0"
            >
          ...
          <div className="flex flex-wrap gap-3 pt-1">
            ...
            >
              <span>📥</span> Download QR Code (PNG)
            </button>
          </div>
        </div>
