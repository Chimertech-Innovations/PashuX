...
export function generateHealthReportPDF(data: ReportPDFData) {
  try {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.setTextColor(100, 116, 139);
      doc.text(
        `This report and data generated by PashuX AI -- Powered by Chimertech Private Limited -- Page ${i} of ${pageCount}`,
        pageWidth / 2,
        287,
        { align: 'center' }
      );
  ...
  const html = `
    ...
    <body>
      ...
      <div style="margin-top:30px; border-top:1px solid #e2e8f0; padding-top:14px; text-align:center; font-size:11px; font-weight:bold; color:#475569;">
        This report and data generated by PashuX AI (Powered by Chimertech Private Limited).
      </div>
    </body>
    </html>
  `;
